"""
This file was populated by copying code cells from dfs_new.ipynb exactly as requested.
Do not modify here if you intend to preserve original notebook content.
"""
...
# This code block loads flight data from a JSON file named 'flight_dataset'
...
"""
def eliminate_backward_travel(first_layover, second_layover, destination, distance_data)-> bool:
    #Function to eliminate backward travel (zig-zag routes)
    
    try:
        # If next layover is the destination, always allow it
        if second_layover == destination:
            return True
        
        # Get distances from first and second layovers to destination
        distance_current = distance_data[first_layover][destination]
        distance_next = distance_data[second_layover][destination]
        
       #if moving closer to destination, allow it
        if distance_next < distance_current:
            return True
        
        #Backward travel case - to be rejected
        else:
            return False
...
def dfs_routes_iterative(flights_graph: Graph, 
    ...
    #Iterative DFS algorithm to enumerate all possible routes from start to destination
    ...
    for next_airport, flight in starting_flights:
        ...
        #DFS         
        while stack:
            current_airport, path, visited, arrival_time = stack.pop()
...
def dfs_routes_iterative(flights_graph: Graph, 
    ...
    if user_departure_time is None:
        starting_flights = flights_graph.get(start, [])
        # if travel_day is not None:
        #     starting_flights = [(dest, f) for dest, f in starting_flights 
        #                       if travel_day in f.get('days', [])]
    else:
        starting_flights = [
            (destination, flight)
            for destination, flight in flights_graph.get(start, [])
            if parse_time(flight['departure_time_gmt']) >= parse_time(user_departure_time)
        ]
        
    # Filter starting flights by travel day if specified    
    if travel_day is not None:
        starting_flights = [(destination, flight) for destination, flight in starting_flights 
                            if travel_day in flight.get('days', [])]
        
    #Stack-based DFS   
    # Iterate over each starting flight     
    for destination, flight in starting_flights:
        stack = [(destination, 
                [(destination, flight)], #List of path
                {start, destination},  #Set of visited airports
                flight['arrival_time_gmt'])] #Arrival time of last flight
        ...
        while stack: #While stack is not empty
            ...
            # Limit layovers (to avoid excessive paths. Max layovers = 3)            
            if len(path) >= max_layovers:
                #path must have start and end airport plus max_layovers
                continue
            ...
            for next_airport, next_flight in next_flights:
                ...
                #Build updated path and visited set   
                new_visited = visited | {next_airport} #Add next airport to visited set
                new_path = path + [(next_airport, next_flight)] #Append next flight to path
...
def print_routes_formatted(routes: List[List[Tuple[str, Flight]]], start: str):
    #Print routes in detailed format
    ...
    for i, route in enumerate(routes, 1):
        ...
        #Build list of airports in the route
        airports = [start] + [leg[0] for leg in route]
        print(f"\n{'='*50}") #fancy formatting for clarity
        ...
        for airport, flight in route:
            ...
            total_cost =  total_cost + flight.get('cost', 0)
            
            #format days of operation
...
if __name__ == "__main__":
    ...
    # Find routes using recursive DFS
    routes = dfs_routes_iterative(graph, distance_data, source, destination, 
                       max_layovers=max_layovers, min_layover=min_layover, 
                       travel_day=travel_day)

    print(f"Found {len(routes)} route(s) using Recursive DFS")
    print_routes_formatted(routes, source)


    # Find routes using iterative DFS
    routes_iterative = dfs_routes_iterative(graph, distance_data, source, destination, 
                                           max_layovers=max_layovers, min_layover=min_layover, 
                                           travel_day=travel_day)

    print(f"Found {len(routes_iterative)} route(s) using Iterative DFS")
    ...
    routes_any_day = dfs_routes_iterative(graph, distance_data, source, destination, 
                               max_layovers=max_layovers, min_layover=min_layover, 
                               travel_day=None)
    ...
    routes_with_time = dfs_routes_iterative(graph, distance_data, source, destination, 
                                 max_layovers=max_layovers, min_layover=min_layover, 
    ...
    for day in days_to_check:
        routes_by_day = dfs_routes_iterative(graph, distance_data, source, destination, 
                                   max_layovers=max_layovers, min_layover=min_layover, 
                                   travel_day=day)
