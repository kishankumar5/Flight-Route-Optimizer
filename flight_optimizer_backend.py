...
def calculate_total_travel_time(route: List[Tuple[str, Flight]], start: str) -> Tuple[int, int]:
    ...
    # Add flight duration for each leg
    for airport, flight in route:
        total_flight_time += flight['duration_mins']
...
def calculate_layover_duration(arrival_time: str, next_leg_time: str) -> float: #returns duration in float
    # Calculate layover duration in minutes between arrival and next leg departure
...
def connection_validity(arrival: str, departure: str, minimum_layover: int = 90) -> bool: 
    # returns true/false depending on layover >= 90 minutes
...
def print_routes_formatted(routes: List[List[Tuple[str, Flight]]], start: str):
    ...
    for i, route in enumerate(routes, 1):
        ...
        for j, (airport, flight) in enumerate(route):
            ...
            # Calculate and display layover time if not the last flight
            if j < len(route) - 1:
